...
var AnimationHelpers = module.exports = {
  ...
  //  0 -> 1 (in) -> 1
  increaseOnEnter: function increaseOnEnter(index, step, easing) {
    var val = Math.min(1, step - index + 1);
    return easing ? Easings[easing](val) : val;
  },

  // Decrease strength on exit
  //  1 -> 1 (in) -> 0
  decreaseOnExit: function decreaseOnExit(index, step, easing) {
    var val = Math.max(0, 1 - (step - index));
    return easing ? Easings[easing](val) : val;
  },

  // Decrease strength on enter
  //  1 -> 1 (in) -> 0
  decreaseOnEnter: function decreaseOnEnter(index, step, easing) {
    var val = Math.min(1, index - step);
    return easing ? Easings[easing](val) : val;
  },

  // Linear increasing then decreasing strength
  //  0 -> 1 (in) -> 0
  symmetrical: function symmetrical(index, step, easing) {
    return step <= index ? AnimationHelpers.increaseOnEnter(index, step, easing) : AnimationHelpers.decreaseOnExit(index, step, easing);
  },
  ...
  scaleBetween: function scaleBetween(step, fromTop, fromBot, toTop, toBot) {
    ...
    if (step === pivot) {
      ...
      // ex: step = 1.5, pivot = 1, fromTop = 2, toTop = 1.2, returns 1.1
      return pivot + (step - pivot) * (toTop - pivot);
    } else {
      // ex: step = .1, pivot = 1, fromBot = 0, toBot = .8, returns .805
