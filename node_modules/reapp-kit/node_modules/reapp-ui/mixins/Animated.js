...
exports['default'] = {
  ...
  componentWillUnmount: function componentWillUnmount() {
    if (this.unlistenAnimations) this.unlistenAnimations();
  },
  ...
  componentWillReceiveProps: function componentWillReceiveProps(nextProps) {
    ...
    if (!shallowEqual(nextProps.animationState, this.props.animationState)) this.setAnimationState(nextProps);
  },

  setAnimationState: function setAnimationState(props) {
    this.conditionalAnimations = this.getConditionalAnimations(props);
    var hasAnimationProps = this.conditionalAnimations || props.animations;

    if (!hasAnimationProps || !hasAnimationProps && (!this.state || this.state && !this.state.animations)) {
      return;
    }var source = props.animationSource || this.state && this.state.animationSource;
    var state = this.context.animations && this.context.animations[source];

    if (!state || typeof state.stepper === 'undefined') {
      return;
    }this.unlistenAnimations = state.stepper.onChange(this.updateAnimationStep.bind(this, source));

    var _animationState = _defineProperty({}, source, Object.assign({}, state, this.props.animationState));

    _animationState[source].step = state.stepper.value;

    this.setState({ _animationState: _animationState });
  },

  getConditionalAnimations: function getConditionalAnimations(props) {
    if (!props.conditionalAnimations) {
      return;
    }var propsForAnimations = {};
    var hasPropsForAnimations = false;

    Object.keys(props.conditionalAnimations).forEach(function (key) {
      if (props[key]) {
  ...
  updateAnimationStep: function updateAnimationStep(source, step) {
  ...
  disableAnimation: function disableAnimation() {
    if (!this.state.disableAnimation) this.setState({ disableAnimation: true });
  },

  enableAnimation: function enableAnimation() {
    if (this.state.disableAnimation) this.setState({ disableAnimation: false });
  },
  ...
  getAnimations: function getAnimations(ref) {
    if (this.state && this.state.animations && defined(this.state.animations[ref])) {
      ...
      return this.props.animations && this.props.animations[ref] || this.conditionalAnimations && this.conditionalAnimations[ref];
    }
  },
  ...
  _getAnimationStyle: function _getAnimationStyle(styles, state, animation) {
    ...
    var other = _objectWithoutProperties(_animator, ['scale', 'rotate', 'rotate3d', 'translate']);

    // set styles
    if (other) styles = Object.assign(styles || {}, other);
  ...
  _animationTransformsToString: function _animationTransformsToString(scale, rotate3d, rotate, translate) {
    var matrix = new Matrix();

    if (defined(scale)) matrix.scale3d(scale, scale);

    if (defined(rotate3d)) matrix.rotate3d(rotate3d.x, rotate3d.y, rotate3d.z);else if (defined(rotate)) matrix.rotate(rotate);

    if (defined(translate)) matrix.translate(translate.x, translate.y, translate.z);

    return matrix.getMatrixCSS();
  },

  // grabs state, if not, then props
  _stateOrProps: function _stateOrProps() {
    var _this = this;
